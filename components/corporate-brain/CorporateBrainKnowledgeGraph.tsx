...
const [counts, setCounts] = useState<GraphCounts>({
policies: 0,
objectives: 0,
risks: 0,
teams: 0,
...
async function loadGraphData() {
  ...
  const { count: knowledgeSourcesCount } = await supabase
    .from("knowledge_sources")
    .select("id", { count: "exact", head: true })
    .eq("company_id", companyId);

  const { count: discoveryAnswersCount } = await supabase
  ...
  const knowledgeSources = knowledgeSourcesCount ?? 0;
  const discoveryAnswers = discoveryAnswersCount ?? 0;
...
icon: Building2,
title: companyName,
subtitle: isArabic ? "المؤسسة" : "Organization",
className:
...
subtitle: isArabic
? `${counts.policies} مصادر`
: `${counts.policies} sources`,
className:
"bg-[var(--brand-subtle)] text-[var(--brand-primary)]",
},
{
icon: Target,
title: isArabic ? "الأهداف" : "Objectives",
subtitle: isArabic
? `${counts.objectives} أهداف`
...
{
icon: UsersRound,
title: isArabic ? "فرق التنفيذ" : "Execution Teams",
subtitle: isArabic
? `${counts.teams} فرق`
: `${counts.teams} teams`,
className:
